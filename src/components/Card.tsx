				...
				<div className="flex flex-col items-start rounded-lg bg-neutral-50 p-8 dark:bg-neutral-800">
					...
					{data.data.uptime && (
						<>
							<p className="mb-4 text-xl font-bold">
								{((data.data.uptime.up / data.data.uptime.all) * 100).toFixed(
									2,
								)}
								%
							</p>
								...
								{[...data.data.uptime.history].reverse().map((check, idx) => (
									<Tooltip.Root delayDuration={250} key={idx}>
										<Tooltip.Trigger asChild>
											<li
												className={twMerge([
													'h-full w-full first:rounded-l-lg last:rounded-r-lg bg-green-400',
													check > 0
														? 'bg-green-400'
														: check === 0
															? 'bg-red-400'
					...
					{data.data.lighthouse && (
						<div className="mt-6 grid grid-cols-2 gap-6 self-stretch lg:grid-cols-4">
							{Object.entries(data.data.lighthouse).map(([key, value]) => (
								<div className="flex flex-col" key={key}>
									<span className="text-sm font-medium">
										{
											readableLighthouseTitle[
												key as keyof typeof readableLighthouseTitle
											]
										}
									</span>
